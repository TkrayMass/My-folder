...
This repository contains the Snowflake SQL used to refresh the **Monthly Budget Report**.

The automation rebuilds the monthly Budget reporting data from source transactions and eligibility data, calculates utilization and PMPM measures, creates Last Twelve Months (LTM) and variance outputs, refreshes the Tableau-facing LTM patch table, and runs automatically on the **15th of every month at 9:00 AM Eastern Time**.

The process is designed so that the monthly refresh does **not require a manual run-date change**. Procedure 1 uses `CURRENT_DATE()` and derives the reporting period dynamically.
...
| Schema | `MHA` |
| Role | `MHA_TEAM_ROLE` |
| Master procedure | `MHTEAM.MHA.BUDGET_MONTHLY_MASTER()` |
| Scheduled task | `MHTEAM.MHA.BUDGET_MONTHLY_MASTER_TASK` |
| Schedule | 15th of every month, 9:00 AM Eastern |
| Time zone | `America/New_York` |
| Failure email integration | `MHA_EMAIL_NI` |
| Failure email recipient | `Tatyana.Kray@mass.gov` |
...
```text
BUDGET_MONTHLY_MASTER_TASK
        |
        v
BUDGET_MONTHLY_MASTER()
        |
        +--> P1  BUILD_TRANSACTIONS
        |
        +--> P2  COMBINE_FEEDS
        |
        +--> P3  BUILD_MEM_MONTHS
        |
        +--> P4  AGGREGATE_MEASURES
        |
        +--> P5  PMPM_MEASURES
        |
        +--> P6  LTM
        |
        +--> ROLL_FORWARD_LTM_PATCH_TABLE
```

The procedures must run in this order because later steps depend on tables created by earlier steps.
...
### Purpose
Builds the core source transaction feeds used by the Monthly Budget Report.
...
- The calculated dates are saved in `BUDGET_MONTHLY_TIME_CONFIG` for downstream procedures.
...
### Why It Matters
This step establishes both the monthly reporting window and the transaction-level source data used by the rest of the process.
...
### Purpose
Combines the separate FFS and managed-care transaction feeds into a single reporting transaction table.

### Main Work
- Combines FFS, managed-care capitation, and other managed-care feeds.
...
### Why It Matters
This becomes the primary transaction-level fact table used by spending, utilization, PMPM, and LTM calculations.
...
### Purpose
Builds eligibility and member-month denominators for the Monthly Budget Report.

### Date Logic
Uses the dates stored in `BUDGET_MONTHLY_TIME_CONFIG` by Procedure 1.
...
### Why It Matters
Member months are required to calculate PMPM measures accurately.
...
### Purpose
Aggregates transaction data into provider-category, provider-group, and provider-type measures.
...
- total paid amounts
- paid per utilizer
- Fee For Service totals
- Programmatic Spend totals
- Tableau ordering fields
...
### Purpose
Creates monthly and quarterly PMPM measures used by the Monthly Budget Report.

### Main Work
Builds PMPM views by:
- demographic
- plan type
- demographic x plan
- demographic x provider category
- plan type x provider category
- overall MassHealth total
- quarter
...
### Quarterly PMPM
Quarterly PMPM uses quarterly spending divided by the corresponding member-month denominator across the quarter.
...
### Purpose
Creates Last Twelve Months reporting, fiscal-year rollups, variance calculations, and monthly frozen LTM snapshot history.

### Main Work
- Detects the latest available reporting month dynamically.
- Builds the current 12-month reporting window.
- Creates LTM provider-category and provider-group totals.
- Creates closed fiscal-year rollups.
- Calculates FY-to-FY and FY-to-LTM variance.
- Adds missing monthly snapshots to frozen LTM history.
...
### Purpose
Refreshes the Tableau-facing comparison between the current LTM period and the prior frozen LTM period.

### Dynamic Month Logic
- Current month is read from `BUDGET_MONTHLY_LTM_STAGE`.
- Prior month is calculated dynamically as current month minus one month.
- No manual month update is required.

### Main Outputs
- `BUDGET_LTM_DOS_SPEND_PATCH_202510_FINAL`
- `BUDGET_LTM_PATCH_HIST`

### Idempotent Behavior
The procedure deletes and reinserts the current month in patch history, so rerunning the same reporting month does not create duplicate history rows.
...
The master procedure is the orchestration layer for the Monthly Budget Report refresh.

It calls each child procedure sequentially:
...
The master procedure records the current step and procedure name.

If a child procedure throws an error:
- execution stops,
- the failed step is identified,
- Snowflake error details are captured,
- elapsed time is captured,
- completed steps are included in the error message,
- a failure email is sent through `MHA_EMAIL_NI` to `Tatyana.Kray@mass.gov`.

If every step completes, the master returns a `SUCCESS` message with start time, end time, elapsed seconds, and the child-procedure return messages.

---

# Scheduled Snowflake Task

## `MHTEAM.MHA.BUDGET_MONTHLY_MASTER_TASK`

The Snowflake Task is responsible only for starting the master procedure.
...
- Snowflake handles EST/EDT automatically through `America/New_York`
...
The child procedures are **not scheduled individually**.
...
1. Snowflake starts `BUDGET_MONTHLY_MASTER_TASK` at 9:00 AM Eastern.
2. The task calls `BUDGET_MONTHLY_MASTER()`.
3. The master runs Procedures 1 through 6 and the LTM patch procedure in order.
4. Procedure 1 calculates the reporting dates dynamically from the current run date.
5. The report tables and views are rebuilt.
6. LTM snapshot history is updated only for a missing reporting month.
7. The Tableau-facing LTM patch table is refreshed.
8. If all steps succeed, the refresh completes normally.
9. If a procedure fails, the master stops and sends a failure email.
...
```text
USING CRON 0 9 15 * * America/New_York
```
...
To manually run the entire Monthly Budget refresh:

```sql
CALL MHTEAM.MHA.BUDGET_MONTHLY_MASTER();
```

**Warning:** This executes the full production refresh chain.
...
For a new or rebuilt environment, deploy in this order:

1. `sql/01_monthly_budget_child_procedures.sql`
2. `sql/02_monthly_budget_master_procedure.sql`
3. `sql/03_monthly_budget_scheduled_task.sql`

The child procedures must exist before the master is executed, and the master must exist before the scheduled task runs.
...
|
+-- sql/
    +-- 01_monthly_budget_child_procedures.sql
    +-- 02_monthly_budget_master_procedure.sql
    +-- 03_monthly_budget_scheduled_task.sql
```
...
The same SQL is also stored as separate files under the `sql/` folder for easier deployment and version control.
...
## Master Procedure With Failure Email
